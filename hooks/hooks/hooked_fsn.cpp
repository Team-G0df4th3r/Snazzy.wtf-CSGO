...
Vector rape; //flb aim punch
Vector children; //flb view punch

Vector* smell; //aim punch
Vector* farts; //view punch
...
void __stdcall hooks::hooked_fsn(ClientFrameStage_t stage)
{
	...
	if (stage == FRAME_RENDER_START)
	{
		...
		{
			smell = &g_ctx.local()->m_aimPunchAngle();
			farts = &g_ctx.local()->m_viewPunchAngle();
			...
			if (!current_shot->latency)
			{
				...
				if (!current_shot->hurt_player)
				{
					...
					if (current_shot->impact_hit_player)
					{
						...
						++g_ctx.globals.missed_shots_reset[current_shot->last_target];
					...
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						...
						if (current_shot->occlusion)
							...
							eventlogs::get().add(crypt_str("Missed shot due to hitchance calculation error"));
						else
	...
	if (g_cfg.player.enable && g_cfg.esp.removals[REMOVALS_RECOIL] && g_ctx.local()->is_alive() && smell && farts)
	{
		*smell = rape;
		*farts = children;
	}
