...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	else if (!strcmp(event_name, crypt_str("weapon_fire")))
	{
		...
		{
			static bool flip = antiaim::get().flip;

			if (g_cfg.antiaim.invert_onshot && flip)
				flip = false;
			else if (g_cfg.antiaim.invert_onshot && !flip)
				flip = true;
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			if (g_cfg.esp.hitsound && g_cfg.player.enable)
			{
				...
				{
				case 1:
					m_surface()->PlaySound_(crypt_str("skeet.wav"));
					break;
				case 2:
					m_surface()->PlaySound_(crypt_str("cod.wav"));
					break;
				case 3:
					m_surface()->PlaySound_(crypt_str("bameware.wav"));
					break;
				case 4:
					m_surface()->PlaySound_(crypt_str("flick.wav"));
					break;
				case 5:
					m_surface()->PlaySound_(crypt_str("snazzy.wav"));
					break;
				case 6:
					m_surface()->PlaySound_(crypt_str("snazzy2.wav"));
					break;
				case 7:
					m_surface()->PlaySound_(crypt_str("bucket.wav"));
					break;
				}
			}
