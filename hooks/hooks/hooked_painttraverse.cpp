...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (!in_game && m_engine()->IsInGame()) //-V807
	{
		...
		for (auto i = 1; i < 65; i++)
		{
			...
			g_ctx.globals.missed_shots_reset[i] = 0;
	...
	if (reload_fonts())
	{
		...
		DWORD Font; //font in the RAM

		AddFontMemResourceEx((void*)undefeated, sizeof(undefeated), NULL, &Font);
		AddFontMemResourceEx((void*)icomoon, sizeof(icomoon), NULL, &Font);
		AddFontMemResourceEx((void*)snazzyfont, sizeof(snazzyfont), NULL, &Font);

		fonts[LOGS] = create_font(crypt_str("Verdana"), 13, FW_HEAVY, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
		fonts[ESP] = create_font(crypt_str("Verdana"), 12, FW_HEAVY, FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
		fonts[NAME] = create_font(crypt_str("Verdana"), 12, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[SUBTABWEAPONS] = create_font(crypt_str("undefeated"), 13, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
		fonts[KNIFES] = create_font(crypt_str("icomoon"), 13, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
		fonts[GRENADES] = create_font(crypt_str("undefeated"), 20, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
		fonts[DAMAGE_MARKER] = create_font(crypt_str("Arial"), 15, FW_HEAVY, FONTFLAG_OUTLINE | FONTFLAG_DROPSHADOW);
		fonts[KEYBINDS] = create_font(crypt_str("snazzyfont"), 20, FW_BOLD, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
		fonts[KEYBINDS1] = create_font(crypt_str("snazzyfont"), 15, FW_BOLD, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[WATERMARK] = create_font(crypt_str("snazzyfont"), 17, FW_BOLD, FONTFLAG_NONE);
		fonts[SNAZZY] = create_font(crypt_str("snazzyfont"), 25, FW_BOLD, FONTFLAG_NONE);
		fonts[INDICATOR] = create_font(crypt_str("Smallest Pixel-7"), 20, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
		fonts[AMMO] = create_font(crypt_str("snazzyfont"), 15, FW_BOLD, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
	...
	if (panel_id == panel)
	{
		if (g_ctx.available())
		{
			...
			{
				if (qgui::get().public_alpha > 0.15f && g_cfg.legitbot.enabled)
				{
					int x, y;
					...
					if (g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].fov)
					{
						...
						render::get().circle_filled(x / 2, y / 2, 60, radius, Color(235, 235, 235, qgui::get().public_alpha * 0.68));
						render::get().circle(x / 2, y / 2, 60, radius, Color(235, 235, 235, qgui::get().public_alpha * 0.8));
					}
					...
					if (g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].silent_fov)
					{
						...
						render::get().circle_filled(x / 2, y / 2, 60, silent_radius, Color(15, 235, 15, qgui::get().public_alpha * 0.68));
						render::get().circle(x / 2, y / 2, 60, silent_radius, Color(15, 235, 15, qgui::get().public_alpha * 0.8));
					}
				}
			}
