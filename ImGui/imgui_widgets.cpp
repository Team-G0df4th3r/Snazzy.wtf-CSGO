...
bool ImGui::ContentTab(const char* label, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 text_act = ImVec4(112 / 255.F, 15 / 255.F, 247 / 255.F, 1.f);
...
bool ImGui::MainTab(const char* label, ImFont* icon_font, const char* icon, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    PushFont(icon_font);
    RenderTextClipped(bb.Min + ImVec2(size_arg.x * 0.05f, -3 * qgui::get().dpi_scale), bb.Max, icon, NULL, &icon_size, ImVec2(0.f, 0.27f), &bb);
    PopFont();
    ...
    PushStyleColor(ImGuiCol_Text, it_text->second);
    RenderTextClipped(bb.Min + ImVec2(size_arg.x * 0.33f, (4 * qgui::get().dpi_scale) - 4), bb.Max, label, NULL, &label_size, ImVec2(0.f, 0.41f), &bb);
    PopStyleColor();
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ...
    //RenderFrame(bb.Min, bb.Max, col, true, 2.f * qgui::get().dpi_scale);
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(112 / 255.F, 15 / 255.F, 247 / 255.F, it_hover->second)), 2.f * qgui::get().dpi_scale);
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ...
    const ImVec4 text_act = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    const ImVec4 icon_act = ImVec4(112 / 255.F, 15 / 255.F, 247 / 255.F, 1.f);

    RenderFrame(bb.Min, bb.Max, col, true, 2.f * qgui::get().dpi_scale);

    window->DrawList->PushClipRect(bb.Min, bb.Max, false);
    ImGui::GetWindowDrawList()->AddCircleFilled(it_circle->second.pos, it_circle->second.radius, GetColorU32(ImVec4(112 / 255.F, 15 / 255.F, 247 / 255.F, it_circle->second.alpha)), 60);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Render
    ImVec4 hover_dis = ImVec4(112 / 255.F, 15 / 255.F, 247 / 255.F, 1.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec4 hover_act = ImVec4(202 / 255.f, 202 / 255.f, 202 / 255.f, 1.f);
    const ImVec4 hover_dis = ImVec4(112 / 255.F, 15 / 255.F, 247 / 255.F, 1.f);
    ...
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second)), 2.f * qgui::get().dpi_scale, 15, 0.7f);

    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(112 / 255.F, 15 / 255.F, 247 / 255.F, it_filled->second)), 2.f * qgui::get().dpi_scale);


    ImGui::PushStyleColor(ImGuiCol_Text, it_text->second);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + 2 * qgui::get().dpi_scale - (1 * (1.8 - qgui::get().dpi_scale))), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
    ...
    const ImVec4 hover_act = ImVec4(112 / 255.F, 15 / 255.F, 247 / 255.F, 1.f);
    ...
    const ImVec4 arrow_act = ImVec4(112 / 255.F, 15 / 255.F, 247 / 255.F, 1.f);
    ...
    PushStyleVar(ImGuiStyleVar_Alpha, it_open->second * qgui::get().public_alpha);
...
bool ImGui::BeginCombo2(const char* label, const char* preview_value, ImGuiComboFlags flags, int items)
{
    // Always consume the SetNextWindowSizeConstraint() call in our early return paths
    ImGuiContext& g = *GImGui;
    ...
    if (label_size.x > 0)
        window->DrawList->AddText(NULL, frame_bb.Min + ImVec2(0, 2), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), label);

    window->DrawList->PushClipRect(cb_bb.Min, cb_bb.Max - ImVec2(20, 0), true);
    window->DrawList->AddText(NULL, cb_bb.Min + ImVec2(8, 4), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), preview_value);
    window->DrawList->PopClipRect();
...
bool ImGui::SingleSelect(const char* label, int* current_item, std::vector<const char*> items) {
    *current_item = ImClamp(*current_item, 0, int(items.size() - 1));
    ...
    if (ImGui::BeginCombo2(label, items.at(*current_item), 0, items.size())) {
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    if (window->DC.CurrLineTextBaseOffset != window->DC.PrevLineTextBaseOffset)
        SetCursorPosY(GetCursorPosY() + qgui::get().dpi_scale * 8.f);
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
bool ImGui::SliderBehaviorT2(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
...
bool ImGui::SliderBehavior2(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    switch (data_type)
    {
    case ImGuiDataType_S8: { ImS32 v32 = (ImS32) * (ImS8*)p_v;  bool r = SliderBehaviorT2<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS8*)p_min, *(const ImS8*)p_max, format, flags, out_grab_bb); if (r)*(ImS8*)p_v = (ImS8)v32;  return r; }
    case ImGuiDataType_U8: { ImU32 v32 = (ImU32) * (ImU8*)p_v;  bool r = SliderBehaviorT2<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU8*)p_min, *(const ImU8*)p_max, format, flags, out_grab_bb); if (r)*(ImU8*)p_v = (ImU8)v32;  return r; }
    case ImGuiDataType_S16: { ImS32 v32 = (ImS32) * (ImS16*)p_v; bool r = SliderBehaviorT2<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS16*)p_min, *(const ImS16*)p_max, format, flags, out_grab_bb); if (r)*(ImS16*)p_v = (ImS16)v32; return r; }
    case ImGuiDataType_U16: { ImU32 v32 = (ImU32) * (ImU16*)p_v; bool r = SliderBehaviorT2<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU16*)p_min, *(const ImU16*)p_max, format, flags, out_grab_bb); if (r)*(ImU16*)p_v = (ImU16)v32; return r; }
    case ImGuiDataType_S32:
        IM_ASSERT(*(const ImS32*)p_min >= IM_S32_MIN / 2 && *(const ImS32*)p_max <= IM_S32_MAX / 2);
    ...
    case ImGuiDataType_U32:
        IM_ASSERT(*(const ImU32*)p_max <= IM_U32_MAX / 2);
        return SliderBehaviorT2<ImU32, ImS32, float >(bb, id, data_type, (ImU32*)p_v, *(const ImU32*)p_min, *(const ImU32*)p_max, format, flags, out_grab_bb);
    case ImGuiDataType_S64:
        IM_ASSERT(*(const ImS64*)p_min >= IM_S64_MIN / 2 && *(const ImS64*)p_max <= IM_S64_MAX / 2);
        return SliderBehaviorT2<ImS64, ImS64, double>(bb, id, data_type, (ImS64*)p_v, *(const ImS64*)p_min, *(const ImS64*)p_max, format, flags, out_grab_bb);
    case ImGuiDataType_U64:
        IM_ASSERT(*(const ImU64*)p_max <= IM_U64_MAX / 2);
        return SliderBehaviorT2<ImU64, ImS64, double>(bb, id, data_type, (ImU64*)p_v, *(const ImU64*)p_min, *(const ImU64*)p_max, format, flags, out_grab_bb);
    case ImGuiDataType_Float:
        IM_ASSERT(*(const float*)p_min >= -FLT_MAX / 2.0f && *(const float*)p_max <= FLT_MAX / 2.0f);
    ...
    case ImGuiDataType_Double:
        IM_ASSERT(*(const double*)p_min >= -DBL_MAX / 2.0f && *(const double*)p_max <= DBL_MAX / 2.0f);
        return SliderBehaviorT2<double, double, double>(bb, id, data_type, (double*)p_v, *(const double*)p_min, *(const double*)p_max, format, flags, out_grab_bb);
    case ImGuiDataType_COUNT: break;
    }
    IM_ASSERT(0);
    return false;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
    ...
    SetCursorPosX(GetCursorPosX() + qgui::get().dpi_scale * 3.f);

    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth() - qgui::get().dpi_scale * 3.f;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 11 * qgui::get().dpi_scale), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f * 0.5f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0));

    ItemSize(ImRect(total_bb.Min - ImVec2(0, 22 * qgui::get().dpi_scale), total_bb.Max), style.FramePadding.y);
    if (!ItemAdd(ImRect(total_bb.Min - ImVec2(0, 22 * qgui::get().dpi_scale), total_bb.Max), id, &frame_bb))
        return false;
    ...
    if (minimum_damage && *(int*)p_data == 0)
    {
        // value_buf_end = crypt_str("Auto");
        value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), "%s", "Auto");
    }
    RenderTextClipped(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x, frame_bb.Min.y - CalcTextSize(value_buf).y - (4 * qgui::get().dpi_scale)), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.0f, 0.0f));

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - CalcTextSize(label).y - (4 * qgui::get().dpi_scale)), label);
...
bool ImGui::ColorEdit(const char* label, Color* newcolor, ImGuiColorEditFlags flags, bool scripts)
{
    ...
    ImGui::SetCursorPosX(ImGui::GetWindowSize().x - (ImGui::GetWindowSize().x - ImGui::CalcItemWidth()) - 24 * qgui::get().dpi_scale);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(24 * qgui::get().dpi_scale, 17 * qgui::get().dpi_scale)))
        {
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    SetCursorPosY(GetCursorPosY() - 2 * qgui::get().dpi_scale);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ImGui::SetCursorPosX(ImGui::GetCursorPosX() - 4 * qgui::get().dpi_scale);
        ...
        {
            memcpy(qgui::get().color_buffer, col, components * sizeof(float));
        }

        if (CustomButton("Paste", "##COPY_PASTE", ImVec2(square_sz * 2.57f, 0)))
        {
            memcpy(col, qgui::get().color_buffer, components * sizeof(float));
            value_changed = true;
        }
...
bool ImGui::CustomSelectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImGui::SetWindowFontScale(qgui::get().dpi_scale);
