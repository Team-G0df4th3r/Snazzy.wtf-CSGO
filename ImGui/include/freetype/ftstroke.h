...
FT_BEGIN_HEADER
   ...
   *
   * @description:
   *   These values determine how the end of menu_open sub-paths are rendered in
   *   a stroke.
   *
   * @values:
   ...
   *
   * @note:
   *   If `menu_open` is~0 (the default), the outline is treated as a closed
   *   path, and the stroker generates two distinct 'border' outlines.
   *
   *   If `menu_open` is~1, the outline is processed as an open path, and the
   ...
   *   subpath was not 'menu_open', this function 'draws' a single line segment
   ...
   *
   *   When the outline, or a sub-path, is 'menu_open', the stroker merges the
