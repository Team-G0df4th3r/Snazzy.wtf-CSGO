...
void TextEditor::Render()
{
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			drawList->AddText(NULL, ImVec2(lineStartScreenPos.x + mTextStart - lineNoWidth, lineStartScreenPos.y), mPalette[(int)PaletteIndex::LineNumber], buf);
			...
			for (int i = 0; i < line.size();)
			{
				...
				if ((color != prevColor || glyph.mChar == '\t' || glyph.mChar == ' ') && !mLineBuffer.empty())
				{
					...
					drawList->AddText(NULL, newOffset, prevColor, mLineBuffer.c_str());
