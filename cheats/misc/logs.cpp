...
void eventlogs::paint_traverse()
{
	...
	for (size_t i = 0; i < logs.size(); i++)
	{
		...
		auto logwidth = render::get().text_width(fonts[LOGS], log.message.c_str());

		auto logs_size_inverted = 10 - logs.size();
		//render::get().rect_filled(log.x - 5, last_y + log.y - logs_size_inverted * 14 - 5 + offset, logwidth + 20,last_y + log.y - logs_size_inverted * 11.5 + 5, Color(40, 40, 40));
		//render::get().rect_filled(log.x, last_y + log.y - logs_size_inverted * 14 + offset, logwidth + 10,last_y + log.y - logs_size_inverted * 11.5 - 5, Color(20, 20, 20));
...
void eventlogs::events(IGameEvent* event) 
{
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_HIT] && !strcmp(event->GetName(), crypt_str("player_hurt")))
	{
		...
		{
			ss << crypt_str("Received ") << event->GetInt(crypt_str("dmg_health")) << crypt_str(" damage from ") << attacker_info.szName << crypt_str(" in the ") << get_hitgroup_name(event->GetInt(crypt_str("hitgroup")));
...
#if RELEASE
#if BETA
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("Snazzy v3 BETA: ")); //-V807
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("Snazzy v3: "));
#endif
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("Snazzy v3 ALPHA: ")); //-V807
#endif
...
#if RELEASE
#if BETA
		auto log = crypt_str("[ \x0CASnazzy v3 BETA: \x01] ") + text;
		chat->chat_print(log.c_str());
#else
		auto log = crypt_str("[ \x0C^Snazzy v3: \x01] ") + text;
		chat->chat_print(log.c_str());
#endif
#else
		auto log = crypt_str("[ \x0C^Snazzy v3 ALPHA: \x01] ") + text;
		chat->chat_print(log.c_str());
#endif
	}
}