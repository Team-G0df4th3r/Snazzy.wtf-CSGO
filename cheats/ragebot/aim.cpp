...
void aim::scan(adjust_data* record, scan_data& data, const Vector& shoot_position, bool optimized)
{
	...
	{
		if (g_cfg.ragebot.weapon->prefer_body_aim == false)
		{
			g_cfg.ragebot.weapon->prefer_body_aim == true;
		}
		else
		{
			g_cfg.ragebot.weapon->prefer_body_aim == true;
	...
	else
	{
		if (g_cfg.ragebot.weapon->prefer_body_aim == false)
		{
			g_cfg.ragebot.weapon->prefer_body_aim == false;
		}
		else
		{
			g_cfg.ragebot.weapon->prefer_body_aim == true;
...
void aim::fire(CUserCmd* cmd)
{
	...
	if (g_ctx.globals.double_tap_aim && g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_amount)
		hitchance_amount = 0;
	else if (!g_ctx.globals.double_tap_aim && g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_amount)
		hitchance_amount = g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_amount;

	if (!g_cfg.ragebot.autoscope && g_ctx.globals.weapon->m_zoomLevel() == 0 && (g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_SCAR20 || g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_G3SG1))
	...
	if (net_channel_info)
	{
		...
		if (g_cfg.ragebot.double_tap && g_cfg.ragebot.double_tap_key.key > KEY_NONE && g_cfg.ragebot.double_tap_key.key < KEY_MAX && misc::get().double_tap_key)
		{
			...
			{
				original_tickbase += min(15, max_tickbase_shift);
			}
		}
