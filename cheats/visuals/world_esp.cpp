...
void worldesp::grenade_projectiles(entity_t* entity)
{
	...
	if (name.find("thrown") != std::string::npos ||
		client_class->m_ClassID == CBaseCSGrenadeProjectile || client_class->m_ClassID == CDecoyProjectile || client_class->m_ClassID == CMolotovProjectile || client_class->m_ClassID == CSmokeGrenadeProjectile || client_class->m_ClassID == CIncendiaryGrenade || client_class->m_ClassID == CFlashbang)
	{
		...
		float distance = g_ctx.local()->GetAbsOrigin().DistTo(grenade_origin) / 12.0f;
		...
		Box box;
		util::get_bbox(entity, box, false);

		if (client_class->m_ClassID == CBaseCSGrenadeProjectile || client_class->m_ClassID == CIncendiaryGrenade || client_class->m_ClassID == CMolotovGrenade)
		{
			render::get().circle_filled(grenade_position.x, grenade_position.y, 100, 35, lethal ? Color::Red : Color(40, 40, 40, 200));
			render::get().circle_filled(grenade_position.x, grenade_position.y, 100, 30, Color(20, 20, 20, 250));
		}
		else
		{
			render::get().circle_filled(grenade_position.x, grenade_position.y, 100, 35, Color(40, 40, 40, 200));
			render::get().circle_filled(grenade_position.x, grenade_position.y, 100, 30, Color(20, 20, 20, 250));
		}
			...
			if (client_class->m_ClassID == CBaseCSGrenadeProjectile || client_class->m_ClassID == CIncendiaryGrenade || client_class->m_ClassID == CMolotovGrenade)
			{
				render::get().text(fonts[ESP], box.x + box.w / 2, box.y - 5, lethal ? Color::Red : g_cfg.esp.projectiles_color, HFONT_CENTERED_X, "%i FT", (int)distance);
			}
			else
			{
				render::get().text(fonts[ESP], box.x + box.w / 2, box.y - 5, g_cfg.esp.projectiles_color, HFONT_CENTERED_X, "%i FT", (int)distance);
			}

			render::get().text(fonts[GRENADES], box.x + box.w / 2, box.y - 25, g_cfg.esp.projectiles_color, HFONT_CENTERED_X, grenade_icon.c_str());

			render::get().text(fonts[ESP], box.x + box.w / 2, box.y + box.h + 2, g_cfg.esp.projectiles_color, HFONT_CENTERED_X, grenade_name.c_str());

			/*if (g_cfg.esp.grenade_trail)
			{
				static auto grenade_trail = m_cvar()->FindVar(crypt_str("cl_sim_grenade_trajectory"));
				grenade_trail->GetBool();
				grenade_trail->SetValue(true);
			}*/
