...
void playeresp::draw_health(player_t* m_entity, const Box& box, const HPInfo& hpbox)
{
	...
	render::get().rect_filled(n_box.x, n_box.y - 1, 2.5, n_box.h + 2, back_color);
	render::get().rect_filled(n_box.x, n_box.y + offset, 2.5, bar_height, color);

	auto height = n_box.h - hpbox.hp * n_box.h / 100;

	render::get().text(fonts[ESP], n_box.x - 5, n_box.y + offset + 1, text_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, std::to_string(hpbox.hp).c_str());
}
...
bool playeresp::draw_ammobar(player_t* m_entity, const Box& box)
{
	...
	auto max_ammo = weapon_info->iMaxClip1;
	...
	std::string ammunition = std::to_string(ammo) + " / " + std::to_string(max_ammo);

	if (reloading)
	{
		render::get().text(fonts[AMMO], n_box.x + bar_width / 2, n_box.y + 1, Color(227, 29, 18, color.a()), HFONT_CENTERED_X | HFONT_CENTERED_Y, "RELOADING");
	}
	else
	{
		render::get().text(fonts[AMMO], n_box.x + bar_width / 2, n_box.y + 1, color, HFONT_CENTERED_X | HFONT_CENTERED_Y, ammunition.c_str());
	}
