...
DWORD WINAPI main(PVOID base)
{
	if (License.CheckLicense())
	{
		...
		folder = std::string(path) + crypt_str("\\Snazzy\\");
...
__forceinline void setup_render()
{
	...
	fonts[SUBTABWEAPONS] = create_font(crypt_str("undefeated1"), 13, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[KNIFES] = create_font(crypt_str("icomoon1"), 13, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[GRENADES] = create_font(crypt_str("undefeated1"), 20, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[DAMAGE_MARKER] = create_font(crypt_str("Arial"), 15, FW_MEDIUM, FONTFLAG_OUTLINE);
