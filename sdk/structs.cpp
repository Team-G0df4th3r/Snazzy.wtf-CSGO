...
void __cdecl fDetours::CL_Move(float accumulated_extra_samples, bool bFinalTick)
{
	...
	if (g_ctx.globals.should_teleport)
	{
		static auto cl_forwardspeed = m_cvar()->FindVar(crypt_str("cl_forwardspeed"));
		static auto cl_sidespeed = m_cvar()->FindVar(crypt_str("cl_sidespeed"));
		auto cmd = g_ctx.globals.teleport_cmd;

		if (g_ctx.globals.original_forwardmove >= 1.7f)
			cmd->m_forwardmove = cl_forwardspeed->GetFloat();
		else if (g_ctx.globals.original_forwardmove <= -1.7f)
			cmd->m_forwardmove = -cl_forwardspeed->GetFloat();

		if (g_ctx.globals.original_sidemove >= 1.7f)
			cmd->m_sidemove = cl_sidespeed->GetFloat();
		else if (g_ctx.globals.original_sidemove <= -1.7f)
			cmd->m_sidemove = -cl_sidespeed->GetFloat();

		g_ctx.globals.should_teleport = false;
		return;
	}
...
float player_t::get_max_desync_delta()
{
	...
	auto speedfactor = math::clamp(animstate->m_flFeetSpeedForwardsOrSideWays, 0.0f, 1.01f);
	auto avg_speedfactor = (animstate->m_flStopToFullRunningFraction * -0.301f - 0.201f) * speedfactor + 1.01f;

	auto duck_amount = animstate->m_fDuckAmount;

	if (duck_amount) //-V550
	{
		auto max_velocity = math::clamp(animstate->m_flFeetSpeedUnknownForwardOrSideways, 0.0f, 1.01f);
		auto duck_speed = duck_amount * max_velocity;

		avg_speedfactor += duck_speed * (0.501f - avg_speedfactor);
	}
